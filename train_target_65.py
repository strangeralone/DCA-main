"""目标域自适应脚本（65 类设置）。

实现论文《Dual Classifier Adaptation: Source-Free UDA via Adaptive Pseudo-labels Learning》
在 Office-Home 65 类场景下的无源域自适应阶段。
"""
import argparse
import os, sys
import os.path as osp
import torchvision
import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim
from torchvision import transforms
import network_new
from network_new import *
from torch.utils.data import DataLoader
from data_list import ImageList, ImageList_idx
import random, pdb, math, copy
from loss import *
import torch.nn.functional as F
from utils import *
import os
os.environ['CUDA_LAUNCH_BLOCKING'] = "1"

import clip


def op_copy(optimizer):
    """Store each param group's initial learning rate for scheduling."""
    for param_group in optimizer.param_groups:
        param_group['lr0'] = param_group['lr']
    return optimizer


def lr_scheduler(optimizer, iter_num, max_iter, gamma=10, power=0.75):
    """Polynomial LR schedule used in the original DCA implementation."""
    decay = (1 + gamma * iter_num / max_iter) ** (-power)
    for param_group in optimizer.param_groups:
        param_group['lr'] = param_group['lr0'] * decay
        param_group['weight_decay'] = 5e-4
        param_group['momentum'] = 0.9
        param_group['nesterov'] = True
    return optimizer


def data_load(args):
    """Create target dataloaders for adaptation and evaluation."""
    train_transform = transforms.Compose([
        transforms.Resize((256, 256)),
        transforms.RandomCrop(224),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ])
    test_transform = transforms.Compose([
        transforms.Resize((256, 256)),
        transforms.CenterCrop(224),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ])
    ## prepare data
    dsets = {}
    dset_loaders = {}
    train_bs = args.batch_size
    txt_tar = open(args.t_dset_path).readlines()
    txt_test = open(args.test_dset_path).readlines()

    dsets["target"] = ImageList_idx(txt_tar, transform=train_transform)
    dset_loaders["target"] = DataLoader(
        dsets["target"],
        batch_size=train_bs,
        shuffle=True,
        num_workers=args.worker,
        drop_last=False,
    )
    num_examp = len(dsets["target"])
    dsets["test"] = ImageList(txt_test, transform=test_transform)
    dset_loaders["test"] = DataLoader(
        dsets["test"],
        batch_size=train_bs * 3,
        shuffle=False,
        num_workers=args.worker,
        drop_last=False,
    )

    return dset_loaders, num_examp


def print_args(args):
    """Pretty-print argparse.Namespace for logging."""
    s = "==========================================\n"
    for arg, content in args.__dict__.items():
        s += "{}:{}\n".format(arg, content)
    return s


def train_target_mme(args):
    """Perform source-free adaptation with dual classifiers and pseudo labels."""
    dset_loaders, num_examp = data_load(args)
    ## set base network

    netG = network_new.ResBase(res_name=args.net).to(args.device)
    netF = network_new.bottleneck(type=args.classifier, feature_dim=netG.in_features, bottleneck_dim=args.bottleneck).to(args.device)
    netC = network_new.classifier_C(type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck).to(args.device)
    netD = network_new.classifier_D(type=args.layer, feature_dim=netG.in_features, class_num=args.class_num).to(args.device)


    args.modelpath = args.output_dir_src + "/source_G" + ".pt"
    netG.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + "/source_F" + ".pt"
    netF.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + "/source_C" + ".pt"
    netC.load_state_dict(torch.load(args.modelpath))
    args.modelpath = args.output_dir_src + "/source_D" + ".pt"
    netD.load_state_dict(torch.load(args.modelpath))

    # === 【新增 2】加载并冻结 CLIP 模型 ===
    print("Loading CLIP model...")
    # 使用 ViT-B/16 效果最好，也最常用
    clip_model, preprocess = clip.load("ViT-B/16", device=args.device)
    clip_model.eval()  # 设置为评估模式
    for param in clip_model.parameters():
        param.requires_grad = False  # 冻结参数，不更新 CLIP
    print("CLIP model loaded!")

    netC.eval()
    netD.train()

    for k, v in netC.named_parameters():
        v.requires_grad = False

    param_group_g = []
    param_group_d = []

    learning_rate = args.lr
    for k, v in netG.named_parameters():
        param_group_g += [{"params": v, "lr": learning_rate * 0.1}]
    for k, v in netF.named_parameters():
        param_group_g += [{"params": v, "lr": learning_rate * 1.0}]
    for k, v in netD.named_parameters():
        param_group_d += [{"params": v, "lr": learning_rate * 2.0}]

    # optimizer_g = optim.SGD(param_group_g, momentum=0.9, weight_decay=5e-4, nesterov=True)
    # optimizer_d = optim.SGD(param_group_d, momentum=0.9, weight_decay=5e-4, nesterov=True)

    optimizer_g = optim.SGD(param_group_g)
    optimizer_d = optim.SGD(param_group_d)

    optimizer_g = op_copy(optimizer_g)
    optimizer_d = op_copy(optimizer_d)

    iter_num = 0
    iter_target = iter(dset_loaders["target"])
    max_iter = (args.max_epoch) * len(dset_loaders["target"])
    interval_iter = max_iter // args.interval
    # len(dset_loaders["target"]) = 16


    # 在训练循环之前
    classname_path = "data/officehome/classname.txt"
    with open(classname_path, 'r') as f:
        class_names = [line.strip() for line in f.readlines()]

    # 生成 CLIP 文本 prompt 并预计算 text_features（只需一次）
    prompts = [f"a photo of a {name.replace('_', ' ')}" for name in class_names]
    text_tokens = clip.tokenize(prompts).to(args.device)
    with torch.no_grad():
        text_features = clip_model.encode_text(text_tokens)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    print(f"CLIP text features ready: {text_features.shape}")

    while iter_num < max_iter:
        try:
            inputs_test, _, tar_idx = iter_target.next()
            # inputs_test.size---->torch.Size([bs, 3, 224, 224])
            # _ 表示的是不可获得的label,共64个=batch-size
            # tar_idx 表示的是索引
        except:
            iter_target = iter(dset_loaders["target"])
            inputs_test, _, tar_idx = next(iter_target)

        if inputs_test.size(0) == 1:
            continue

        if iter_num % interval_iter == 0 and args.cls_par > 0:
            netG.eval()
            netF.eval()
            # get the pseudo labels
            mem_label1 = obtain_label(dset_loaders["test"], netG, netF, netC, device=args.device)
            mem_label2 = obtain_label_easy(dset_loaders["test"], netG, netD, device=args.device)
            mem_label1 = torch.from_numpy(mem_label1).to(args.device)
            mem_label2 = torch.from_numpy(mem_label2).to(args.device)
            # mem_label.size = torch.Size([795]) 由795个标签组成的张量。
            # high_label.size 不断更新，从400多到700多，伪标签逐渐准确。
            netG.train()
            netF.train()

        inputs_test = inputs_test.to(args.device)
        batch_size = inputs_test.shape[0]

        iter_num += 1
        lr_scheduler(optimizer_g, iter_num=iter_num, max_iter=max_iter)
        lr_scheduler(optimizer_d, iter_num=iter_num, max_iter=max_iter)

        # Step A Train target date use CELoss
        total_loss1 = 0
        features_d = netG(inputs_test)
        features = netF(features_d)
        # features_test.shape ---->torch.Size([bs, 2048])
        outputs1 = netC(features)
        outputs2 = netD(features_d)
        # outputs_test.shape ---->torch.Size([bs, 31(num_class)])
        softmax_out1 = nn.Softmax(dim=1)(outputs1)
        softmax_out2 = nn.Softmax(dim=1)(outputs2)

        # loss of classifier discrepancy

        loss_skl = torch.mean(torch.sum(SKL(softmax_out1, softmax_out2), dim=1))

        total_loss1 += loss_skl * 0.1

        loss_ent = entropy(netD, features_d, args.lamda)

        total_loss1 += loss_ent

        optimizer_d.zero_grad()
        total_loss1.backward()
        optimizer_d.step()

        # ==================== Step B: 联合训练 backbone + netD ====================
        # 这一步更新 netG (backbone) 和 netD (分类器2)，使用多种损失联合优化
        for _ in range(1):  # 循环1次（可以增加以加强训练）
            total_loss2 = 0  # 初始化 Step B 的总损失
            
            # ===== 前向传播 =====
            features_d = netG(inputs_test)  # backbone 提取特征 [bs, 2048]
            features = netF(features_d)      # 通过 bottleneck 降维 [bs, 256]

            outputs1 = netC(features)        # 分类器1 (冻结) 的预测 [bs, 65]
            outputs2 = netD(features_d)      # 分类器2 的预测 [bs, 65]

            # 将 logits 转换为概率分布
            softmax_out1 = nn.Softmax(dim=1)(outputs1)  # [bs, 65] 概率分布
            softmax_out2 = nn.Softmax(dim=1)(outputs2)  # [bs, 65] 概率分布

            # ===== 损失1: 伪标签分类损失 (带不确定性加权) =====
            # 获取当前 batch 样本对应的伪标签
            pred1 = mem_label1[tar_idx]  # netC 生成的伪标签 [bs]
            pred2 = mem_label2[tar_idx]  # netD 生成的伪标签 [bs]
            
            # 标准交叉熵损失
            classifier_loss1 = nn.CrossEntropyLoss()(outputs1, pred1)  # Head1 的分类损失
            classifier_loss2 = nn.CrossEntropyLoss()(outputs2, pred2)  # Head2 的分类损失

            # ===== 计算不确定性权重 (DCA 核心创新) =====
            # 通过两个分类器的 KL 散度来衡量预测的不确定性
            kl_distance = nn.KLDivLoss(reduction='none')  # 逐元素 KL 散度
            log_sm = nn.LogSoftmax(dim=1)
            
            # variance1: netC 预测与 netD 预测的 KL 差异 [bs]
            variance1 = torch.sum(kl_distance(log_sm(outputs1), softmax_out2), dim=1)
            # variance2: netD 预测与 netC 预测的 KL 差异 [bs]
            variance2 = torch.sum(kl_distance(log_sm(outputs2), softmax_out1), dim=1)
            
            # exp(-variance): 差异越大，权重越小（接近0）；差异越小，权重越大（接近1）
            exp_variance1 = torch.mean(torch.exp(-variance1))  # 平均不确定性权重
            exp_variance2 = torch.mean(torch.exp(-variance2))

            # threshold = 0.5
            # 改成动态阈值
            threshold = variance1.mean() + variance1.std() * 1
            high_discrepancy_mask = variance1 > threshold  # 布尔掩码
            topk_indices = torch.where(high_discrepancy_mask)[0] 

            if len(topk_indices) > 0:  # 只有存在高分歧样本时才触发 CLIP
                clip_input = F.interpolate(inputs_test[topk_indices], size=(224, 224), mode='bicubic')
                # 开始推理
                
                with torch.no_grad():
                    image_features = clip_model.encode_image(clip_input)
                    image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                    # text_features 已在循环外预计算，直接使用

                    # 图像特征和文本特征做点积 → 相似度 → 分类logits
                    clip_logits = image_features @ text_features.T  # [k, 65]
                    clip_probs = F.softmax(clip_logits * 100, dim=1)  # 乘100是CLIP的温度系数
                    # clip_logits 是余弦相似度（范围 -1 到 1），乘 100 是因为 softmax 需要较大的数值差异才能产生明显的概率分布，否则所有类别概率都接近 1/65

                # ===== 计算 CLIP 蒸馏损失（必须在 no_grad 外面！）=====
                # 让两个分类器向 CLIP 的预测靠拢
                log_sm = nn.LogSoftmax(dim=1)
                # .detach() 确保 CLIP 不被更新，但梯度能流向 outputs1/outputs2
                loss_clip = F.kl_div(log_sm(outputs1[topk_indices]), clip_probs.detach(), reduction='batchmean') + \
                            F.kl_div(log_sm(outputs2[topk_indices]), clip_probs.detach(), reduction='batchmean')
            else:
                loss_clip = torch.tensor(0.0).to(args.device)
                
            # 把 CLIP 损失加到总损失（放到 total_loss2 += loss_cs 之后）
            total_loss2 += 0.1 * loss_clip  # 0.1 是权重，可以调

            
            # 加权损失: 分类损失 × 不确定性权重 + 正则化项
            # 如果两个分类器分歧大，伪标签不可靠，分类损失权重小
            loss_seg1 = classifier_loss1 * exp_variance1 + torch.mean(variance1)
            loss_seg2 = classifier_loss2 * exp_variance2 + torch.mean(variance2)

            # 组合两个分类器的损失，alpha 控制权重分配 (默认 0.5，即 1:1)
            classifier_loss = args.alpha * loss_seg1 + (2 - args.alpha) * loss_seg2

            # 乘以伪标签损失的整体权重系数
            loss_cs = args.cls_par * classifier_loss  # cls_par 默认 0.15

            total_loss2 += loss_cs

            # ===== 损失2: 对抗性熵损失 (Minimax Entropy, MME) =====
            # 通过梯度反转层最大化熵，鼓励特征具有更好的判别性
            loss_ent1 = adentropy(netC, features, args.lamda)      # Head1 的对抗熵
            loss_ent2 = adentropy(netD, features_d, args.lamda)    # Head2 的对抗熵

            loss_mme = loss_ent1 + loss_ent2  # 总熵损失

            total_loss2 += loss_mme

            # ===== 损失3: 类别平衡损失 =====
            # 防止模型把所有样本都预测成某几个"优势类"
            # 通过最小化平均预测分布的熵来实现类别平衡
            loss_cb1 = class_balance(softmax_out1, args.lamda)  # Head1 的类别平衡
            loss_cb2 = class_balance(softmax_out2, args.lamda)  # Head2 的类别平衡

            loss_cb = loss_cb1 + loss_cb2

            total_loss2 += loss_cb

            # ===== 损失4: MixUp 数据增强损失 =====
            if args.mix > 0:  # 如果启用 MixUp (默认 0.5)
                alpha = 0.3  # Beta 分布参数
                lam = np.random.beta(alpha, alpha)  # 采样混合比例 λ
                
                # 随机打乱索引，用于配对混合
                index = torch.randperm(inputs_test.size()[0]).to(args.device)
                
                # MixUp 输入: x_mix = λ * x + (1-λ) * x_shuffled
                mixed_input = lam * inputs_test + (1 - lam) * inputs_test[index, :]
                
                # MixUp 标签: 使用软标签 (两个分类器预测的混合)
                # detach() 防止梯度流向原始预测
                mixed_softout = (lam * softmax_out1 + (1 - lam) * softmax_out2[index, :]).detach()

                # 对混合输入进行前向传播
                features_mix = netG(mixed_input)
                outputs_mixed1 = netC(netF(features_mix))  # Head1 对混合输入的预测
                outputs_mixed2 = netD(features_mix)         # Head2 对混合输入的预测

                # 转换为概率分布
                outputs_mied_softmax1 = torch.nn.Softmax(dim=1)(outputs_mixed1)
                outputs_mied_softmax2 = torch.nn.Softmax(dim=1)(outputs_mixed2)

                # MixUp 损失: 预测分布应该接近混合后的软标签
                loss_mix1 = args.mix * nn.KLDivLoss(reduction='batchmean')(outputs_mied_softmax1.log(), mixed_softout)
                loss_mix2 = args.mix * nn.KLDivLoss(reduction='batchmean')(outputs_mied_softmax2.log(), mixed_softout)

                loss_mix = loss_mix1 + loss_mix2

            total_loss2 += loss_mix

            # ===== 反向传播与参数更新 =====
            optimizer_g.zero_grad()  # 清空 backbone + bottleneck 的梯度
            optimizer_d.zero_grad()  # 清空 netD 的梯度
            total_loss2.backward()   # 计算所有参数的梯度
            optimizer_g.step()       # 更新 backbone + bottleneck 参数
            optimizer_d.step()       # 更新 netD 参数
            # 注意: netC 被冻结，不参与更新

        # ==================== 定期评估模型性能 ====================
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            # 切换到评估模式 (关闭 Dropout, BN 使用全局统计量)
            netG.eval()
            netF.eval()
            
            # 计算两个分类器在测试集上的准确率
            acc1, acc_list1, accuracy1 = cal_acc(dset_loaders["test"], netG, netF, netC, device=args.device)  # Head1 准确率
            acc2, acc_list2, accuracy2 = cal_acc_easy(dset_loaders["test"], netG, netD, device=args.device)   # Head2 准确率
            
            acc_best1 = accuracy1
            acc_best2 = accuracy2
            
            # 格式化日志信息
            log_str = (
                    "Task: {}, Iter:{}/{}; Accuracy_c = {:.2f}%, Accuracy_d = {:.2f}% ; Lcls : {:.6f}; Lent : {:.6f}".format(
                        args.name,                                    # 任务名称 (如 "AR")
                        iter_num,                                     # 当前迭代次数
                        args.max_epoch * len(dset_loaders["target"]), # 总迭代次数
                        acc_best1,                                    # Head1 (netC) 准确率
                        acc_best2,                                    # Head2 (netD) 准确率
                        loss_cs.data,                                 # 伪标签分类损失
                        loss_mme.data,                                # 熵损失
                    )
                    + "\n"
                    + str(acc_list1)  # 每类准确率列表
            )

            args.out_file.write(log_str + "\n")
            args.out_file.flush()
            print(log_str + "\n")

            netG.train()
            netF.train()

        if args.savemodel:
            torch.save(netG.state_dict(), osp.join(args.output_dir, "target_G" + ".pt"))
            torch.save(netF.state_dict(), osp.join(args.output_dir, "target_F" + ".pt"))
            torch.save(netC.state_dict(), osp.join(args.output_dir, "target_C" + ".pt"))
            torch.save(netD.state_dict(), osp.join(args.output_dir, "target_D" + ".pt"))

    return netG, netF, netC, netD


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DCA on office-home")
    parser.add_argument(
        "--gpu_id", type=str, nargs="?", default="0", help="device id to run"
    )
    parser.add_argument(
        "--device", type=str, nargs="?", default="cuda", help="device to use: cuda, mps, or cpu"
    )
    parser.add_argument("--s", type=int, default=0, help="source")
    parser.add_argument("--t", type=int, default=2, help="target")
    parser.add_argument("--batch_size", type=int, default=32, help="batch_size")  # 128
    parser.add_argument("--worker", type=int, default=4, help="number of workers")
    parser.add_argument(
        "--dset", type=str, default="officehome", choices=["office", "officehome", "visda"]
    )
    parser.add_argument("--lr", type=float, default=0.01, help="learning rate")
    parser.add_argument(
        "--net", type=str, default="resnet50", help="resnet50, resnet101"
    )
    parser.add_argument('--lamda', type=float, default=0.45, metavar='LAM', help='value of lamda')  # 0.1
    parser.add_argument('--cls_par', type=float, default=0.15)  # 0.05
    parser.add_argument("--mix", type=float, default=0.5)
    parser.add_argument("--seed", type=int, default=2077, help="random seed")
    parser.add_argument("--max_epoch", type=int, default=15, help="max iterations")  # 1543
    parser.add_argument("--interval", type=int, default=10)
    parser.add_argument("--alpha", type=float, default=0.5, help="parameter1")
    parser.add_argument("--epsilon", type=float, default=1e-5)
    parser.add_argument("--bottleneck", type=int, default=256)
    parser.add_argument("--layer", type=str, default="dca")
    parser.add_argument("--classifier", type=str, default="bn", choices=["ori", "bn"])
    parser.add_argument("--output", type=str, default="double")
    parser.add_argument("--da", type=str, default="SFDA")
    parser.add_argument('--savemodel', type=bool, default=True)

    args = parser.parse_args()

    if args.dset == "office":
        names = ["amazon", "dslr", "webcam"]
        args.class_num = 31
    elif args.dset == "officehome":
        names = ["Art", "Clipart", "Product", "RealWorld"]
        args.class_num = 65

    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu_id
    SEED = args.seed
    torch.manual_seed(SEED)
    torch.cuda.manual_seed(SEED)
    np.random.seed(SEED)
    random.seed(SEED)
    # torch.backends.cudnn.deterministic = True

    folder = "./data/"
    if args.dset == "office":
        args.s_dset_path = folder + args.dset + "/" + names[args.s] + "_31.txt"
        args.t_dset_path = folder + args.dset + "/" + names[args.t] + "_31.txt"
    elif args.dset == "officehome":
        args.s_dset_path = folder + args.dset + "/" + names[args.s] + "_65.txt"
        args.t_dset_path = folder + args.dset + "/" + names[args.t] + "_65.txt"
    args.test_dset_path = args.t_dset_path

    current_folder = "./ckps/"
    args.output_dir_src = osp.join(
        current_folder, args.da, args.output, args.dset, names[args.s][0].upper()
    )  # # ./ckps/uda/bait/office/A
    args.name_src = names[args.s][0].upper()
    if not osp.exists(args.output_dir_src):
        os.system("mkdir -p " + args.output_dir_src)
    if not osp.exists(args.output_dir_src):
        os.mkdir(args.output_dir_src)

    args.output_dir = osp.join(
        current_folder,
        args.da,
        args.output,
        args.dset,
        names[args.s][0].upper() + names[args.t][0].upper(),
    )
    args.name = names[args.s][0].upper() + names[args.t][0].upper()
    if not osp.exists(args.output_dir):
        os.system("mkdir -p " + args.output_dir)
    if not osp.exists(args.output_dir):
        os.mkdir(args.output_dir)

    args.out_file = open(
        osp.join(args.output_dir, "log_" + ".txt"),
        "w",
    )
    args.out_file.write(print_args(args) + "\n")
    args.out_file.flush()
    train_target_mme(args)
