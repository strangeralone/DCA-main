...
对应论文《Dual Classifier Adaptation: Source-Free UDA via Adaptive Pseudo-labels Learning》
中的 Office-Home 数据集 65 类场景，负责源域预训练与评估。
"""
...
def data_load(args):
    """Build source/target dataloaders with standard DCA augmentations."""
...
def train_source(args):
    """Pre-train source encoder/classifiers and save the best checkpoints."""
    ...
    for k, v in netC.named_parameters():
        param_group_c += [{"params": v, "lr": learning_rate * 2.0}]# 10
    for k, v in netD.named_parameters():
        param_group_d += [{"params": v, "lr": learning_rate * 2.0}]

    # optimizer_g = optim.SGD(param_group_g, momentum=0.9, weight_decay=1e-3, nesterov=True)
    # optimizer_c = optim.SGD(param_group_c, momentum=0.9, weight_decay=1e-3, nesterov=True)
    # optimizer_d = optim.SGD(param_group_d, momentum=0.9, weight_decay=1e-3, nesterov=True)
    ...
    interval_iter = int(args.interval * len(dset_loaders["source_tr"]))
    ...
    while iter_num < max_iter:
        ...
        classifier_loss1 = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth, device=args.device)(outputs_source1, labels_source)
        classifier_loss2 = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth, device=args.device)(outputs_source2, labels_source)
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            netC.eval()
            netD.eval()
            _, acc_list1, accuracy1 = cal_acc(dset_loaders["source_te"], netG, netF, netC, device=args.device)
            _, acc_list2, accuracy2 = cal_acc_easy(dset_loaders["source_te"], netG, netD, device=args.device)
            acc_best = accuracy1
            log_str = (
                "Task: {}, Iter:{}; Accuracy_c = {:.2f}%, Accuracy_d = {:.2f}%".format(
                    args.name_src, iter_num, accuracy1, accuracy2
                )
                + "\n"
                + str(acc_list1)
            )
...
def test_target(args):
    """Evaluate saved source checkpoints on the designated target split."""
    dset_loaders = data_load(args)
    ## set base network
    ...
    acc1, acc_list1, accuracy1 = cal_acc(dset_loaders["test"], netG, netF, netC, device=args.device)
    acc2, acc_list2, accuracy2 = cal_acc_easy(dset_loaders["test"], netG, netD, device=args.device)
    log_str = (
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dset", type=str, default="officehome", choices=["office", "officehome", "visda"])
    ...
    parser.add_argument("--layer", type=str, default="dca", choices=["linear", "wn", "mme", "dca"])  # 分类器结构
    ...
    if args.dset == "office":
        ...
        # args.s_dset_path =  ./data/officehome/Art_65.txt
        args.s_dset_path = folder + args.dset + "/" + names[args.s] + "_65.txt"
        # args.t_dset_path =  ./data/officehome/Art_65.txt
    ...
    train_source(args)
    test_target(args)
    args.out_file = open(osp.join(args.output_dir_src, 'model_test.txt'), 'w')
